...
struct Node
{
	...
	int find_words(const string& str, char a, char b, int l, int r)
	{
		Node* pBranch = this;
		...
		while(!pBranch->leaf)
		{
			int mid = pBranch->start + (pBranch->end - pBranch->start) / 2;
			if (l > mid)		
		...
		{
			int mid = pLeftLeaf->start + (pLeftLeaf->end - pLeftLeaf->start) / 2;
			if (l > mid)
		...
		{
			int mid = pRightLeaf->start + (pRightLeaf->end - pRightLeaf->start) / 2;
			if (r <= mid)
...
const static int s_threshold = 3;
